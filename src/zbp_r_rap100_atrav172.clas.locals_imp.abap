CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      validatecustomer FOR VALIDATE ON SAVE "The addition FOR VALIDATE ON SAVE indicates that the method provides the implementation of a validation executed on save. Validations are always executed on save.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    DATA:
      entity           TYPE STRUCTURE FOR CREATE zr_rap100_atrav172,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap100_atrav172 FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zc_rap100_atrav172 FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  "IMPORTING parameter keys - an internal table containing the keys of the instances the determination will be executed on includes all entities for which keys must be assigned
  "Implicit CHANGING parameter reported - used to return messages in case of failure
  METHOD setStatusToOpen.
    "Read travel instances of the transferred keys
    READ ENTITIES OF zr_rap100_atrav172 IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF zr_rap100_atrav172 IN LOCAL MODE
      ENTITY Travel
        UPDATE SET FIELDS
  ...
  METHOD validateCustomer.
    ...
    "CHANGING parameters on the other hand need failed and reported tables. Failed is a table with information for identifying the data set where error occured
    "Reported is the table for instance-specific messages \*
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE zr_rap100_atrav172.
    ...
    "The parameter structure %param for parameter input is imported by action with parameters.
    ...
    " The EML statement READ ENTITIES … ALL FIELDS WITH CORRESPONDING is used to read
    " all fields of the updated instances from the buffer to fill the input parameter result.
    READ ENTITIES OF zr_rap100_atrav172 IN LOCAL MODE
        ENTITY Travel
        ALL FIELDS WITH
  ...
  METHOD copyTravel.
   DATA:
      travels       TYPE TABLE FOR CREATE zr_rap100_atrav172.
